...
First you describe the computation that you want to see performed: what the inputs, the variables, and the operations
look like. These get created as nodes over a computation graph. This description is all contained within
the block below:

with graph.as_default():
    ...

Then you can run the operations on this graph as many times as you want by calling session.run(), providing it outputs
to fetch from the graph that get returned. This runtime operation is all contained in the block below:
...
def get_logit(tf_train_dataset, weights_to_hidden, biases_w_h, hidden_to_label, biases_h_l):
    hidden_layer = tf.nn.relu(tf.matmul(tf_train_dataset, weights_to_hidden) + biases_w_h)
...
with graph.as_default():
    ...
    # Training computation.
    logits = get_logit(tf_train_dataset, weights_to_hidden, biases_w_h, hidden_to_label, biases_h_l)
    loss = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits(labels=tf_train_labels, logits=logits))
    ...
    train_prediction = tf.nn.softmax(logits)
    valid_prediction = tf.nn.softmax(
        get_logit(tf_valid_dataset, weights_to_hidden, biases_w_h, hidden_to_label, biases_h_l))
    test_prediction = tf.nn.softmax(
        get_logit(tf_test_dataset, weights_to_hidden, biases_w_h, hidden_to_label, biases_h_l))
